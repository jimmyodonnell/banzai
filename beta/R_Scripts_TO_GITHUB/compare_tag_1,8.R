...
ANALYSIS_DIRECTORY <- "~/Documents/Data/IlluminaData/16S/20141020/Analysis_20141023_1328/Reanalysis_20141028_1401_noclust"
...
# two compare tag 1 and tag 8:
twotags<-t(DATA[c(1,8),])

twotags <- twotags[which(rowSums(twotags)!=0),]

hits <- colSums(twotags)-twotags["No hits",]

twotagsHits <- twotags[-which(rownames(twotags)=="No hits"),]

twotagsTopHits <- twotagsHits[which(rowSums(twotagsHits)>=100),]

diversity(twotagsHits, MARGIN=2)

barplot(matrix(t(twotagsTopHits/colSums(twotagsTopHits)), nr=2), beside=TRUE,
	        col=c("aquamarine3","coral"))

barplot(twotagsHits)
barplot(twotagsHits/hits, beside=TRUE)
barplot(twotagsHits/colSums(twotagsHits), beside=TRUE)
